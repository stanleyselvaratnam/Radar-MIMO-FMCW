...
class DataThread(QThread):
    ...
    def synthetic_data_gen(self, seq_len=150, chunk_len=200, amplitude=0.4):
        byte_buf = b""
        seq_count = 0
        direction = True
    ...
    def run(self):
        ...
        for seq, dir in self.iq_stream_converter.process(raw_data_gen):
            ...
            self.update_signal.emit(np.array(seq, dtype=np.complex64))
    ...
    def plot_data(self):
        ...
        if data is not None and len(data) > 0:
            ...
            if self.plot_type == self.PLOT_RAW_SAMPLES:
                ...
                if self.i_curve is None:                # Partie in-phase
                    print("-" * 40)
                    self.plot_RAW(len(data_np),self.data_thread.te,self.PLOT_RAW_SAMPLES,np.max(data_np.real))

                if self.q_curve is None:                # Partie Quadra-phase
                    self.q_curve = self.plot_widget.plot(pen='b', name="Q-Data")
                
                # Tracer les données I et Q
                i_data = np.real(data_np)[-self.num_points:]  # Partie réelle
                q_data = np.imag(data_np)[-self.num_points:]  # Partie imaginaire
                x_axis = np.arange(len(i_data))

                self.i_curve.setData(x_axis, i_data)
                self.q_curve.setData(x_axis, q_data)
